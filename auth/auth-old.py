...
# --- Utility Imports (Consolidated) ---

# Utility functions (Verify and Hash)
from .utils import verify_password
from auth.utils import hash_password # Assuming this is the correct source

# Email utility
from utils.email import send_email 

# Token utilities (if needed for a custom flow)
from utils.reset_token import create_reset_token, verify_reset_token

# Schemas
from .schema import UserLogin, PasswordResetRequest, PasswordResetConfirm 

# Removed: 
# from .models import User # (You access User via models.User, so this isn't strictly needed if User is in auth/models.py)
...
async def forgot_password(
    ...
    # 1. User check
    if not user:
        # It's better practice to return a generic success message even if the 
        # email doesn't exist to avoid revealing registered emails via the API.
        return {"message": "If an account with that email exists, a password reset link has been sent."}

    # 2. Generate Reset Token (JWT)
    # Token includes 'sub' (User ID) and an extra 'reset' flag for security.
    ...
    # The frontend needs the token to authorize the password change.
